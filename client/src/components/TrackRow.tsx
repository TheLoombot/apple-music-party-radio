...
export function TrackRow({ track, trackNumber, rankNumber, added, onAdd, onRemove, unavailable }: Props) {
  return (
    <div className={`flex items-center gap-3 px-4 py-3 border-b border-border/50 last:border-0 hover:bg-surface/50 group ${unavailable ? "opacity-50" : ""}`}>
      {rankNumber != null && (
        <span className="text-xs text-muted w-5 text-right flex-shrink-0 tabular-nums">{rankNumber}</span>
      )}
      {trackNumber != null ? (
