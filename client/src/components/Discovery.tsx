...
export function Discovery({ catalog, queuedIsrcs, userQueuedIds, nowPlayingIds, suggestedIsrcs, queue, onAddTrack, embedded, suggestions, isPrivileged, currentUserId, onVoteSuggestion, onRetractSuggestion, onEnqueueSuggestion, onRemoveSuggestion }: Props) {
  ...
  async function loadRelated(forceNew = false) {
    const candidates = queue.filter(t => t.appleId)
    ...
    try {
      if (forceNew || allRelatedPlaylists.current.length === 0) {
        const pls = await catalog.getRelatedPlaylists(seed.appleId!)
                          ...
                          {searchResults
                            .filter((item, i, arr) => {
                              ...
                              return arr.findIndex(x =>
                                x.kind === "song" &&
                                (x.track.isrc || x.track.appleId) === (item.track.isrc || item.track.appleId)
                              ) === i
                            })
                            .map(item =>
                              item.kind === "song" ? (
